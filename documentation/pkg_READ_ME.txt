...
The following packages must be installed before running the code.
A .yml file (XRF_pyenv_996417.yml), provided in the same directory
as this READ_ME, can be used to set up an environment with the same packages
as those used here. Not all packages are used (i.e. astropy etc.),
but this is just a copy of the environment I was using to run the code.

In particular, having installed Python 3.x.x, run the following to check that
you have the important packages installed:

pip3 install scipy, numpy, pandas, lmfit, matplotlib
...
conda-build               3.21.4           py37haa95532_0
